...
// Primitive picker is string, identifier, or starred expression.
macro primitive {
  rule {   $p:lit    }
  rule {   $p:expr * } => { $p }
...
// May be prefixed with `?` to check object validity.
macro pickee {
  rule { ? $o:expr } => { P.check($o) }
  rule {   $o:expr }
}
...
macro (@) {
  ...
  // Pick array into array (range case)
  ...
  // 'a`! # b
  case infix {   $p:lit    ! | _ $o:expr } => { return #{ P.pickArrToVal($o, P.mandatory(P.pick1($p))) } }

  // 'a` # b
  case infix {   $p:lit      | _ $o:expr } => { return #{ P.pickArrToVal($o, P.pick1($p))              } }

  // [a]! # b
  case infix { $p:expr * !   | _ $o:expr } => { return #{ P.pickArrToVal($o, P.mandatory(P.pick1($p))) } }

  // [a] # b
  case infix { $p:expr *     | _ $o:expr } => { return #{ P.pickArrToVal($o, P.pick1($p))              } }

  // a! # b
  case infix {   $p:ident  ! | _ $o:expr } => {
    letstx $p = [makeValue(unwrapSyntax(#{$p}), #{$p})];
    return #{ P.pickArrToVal($o, P.mandatory(P.pick1($p))) }
  }

  // a # b
  case infix {   $p:ident    | _ $o:expr } => {
    letstx $p = [makeValue(unwrapSyntax(#{$p}), #{$p})];
    return #{ P.pickArrToVal($o, P.pick1($p)) }
  }
...
macro (#) {
  ...
  // 'a`! # b
  case infix {   $p:lit    ! | _ $o:expr } => { return #{ P.pickObjToVal($o, P.mandatory(P.pick1($p))) } }

  // 'a` # b
  case infix {   $p:lit      | _ $o:expr } => { return #{ P.pickObjToVal($o, P.pick1($p))              } }

  // [a]! # b
  case infix { $p:expr * !   | _ $o:expr } => { return #{ P.pickObjToVal($o, P.mandatory(P.pick1($p))) } }

  // [a] # b
  case infix { $p:expr *     | _ $o:expr } => { return #{ P.pickObjToVal($o, P.pick1($p))              } }

  // a! # b
  case infix {   $p:ident  ! | _ $o:expr } => {
    letstx $p = [makeValue(unwrapSyntax(#{$p}), #{$p})];
    return #{ P.pickObjToVal($o, P.mandatory(P.pick1($p))) }
  }

  // a # b
  case infix {   $p:ident    | _ $o:expr } => {
    letstx $p = [makeValue(unwrapSyntax(#{$p}), #{$p})];
    return #{ P.pickObjToVal($o, P.pick1($p)) }
  }
