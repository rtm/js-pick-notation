...
function key(p) {
  ...
  return function(o, pairs) {
    ...
    if (typeof p === 'function') p(o, pairs);
    else pairs.push(...explode() . map((p, i) => make_pair(p, i, o)));
  };
...
function range(i, a, b) {
  return function(o, pairs) {
    ...
    let new_pairs = range . map((k, i) => make_pair(k, i, o));
...
function execute(o) {
  let pairs = [];
  funcs.forEach(func => func(o, pairs));
  return pairs;
}
...
function object(x) {
  return function(o) {
    let pairs = [];
    x(o, pairs);
    return Object.assign({}, ...pairs . filter(not_omitted) . map(make_obj));
  };
}
...
//console.log(Pick().list([Pick().key('a')]).object()({a: 221}));
//console.log(Pick().list([Pick().key('a'), Pick().key('b')]).object()({a: 221}));
console.log(P.object(P.list([
  P.list([
    P.list([P,key('a')]),
    P.key('b')]),
  P.deflt(22)
